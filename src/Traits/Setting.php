...
trait Setting
{
    public function cachedEntitySettings()
    {
        ...
        $cacheKey = 'entity_settings_for_' . $this->$rolePrimaryKey;
    ...
    public function cachedAllowedValues()
    {
        $rolePrimaryKey = $this->primaryKey;
        $cacheKey = 'allowed_values_for_' . $this->$rolePrimaryKey;
        if (Cache::getStore() instanceof TaggableStore && Config::get('holo.cache.enabled')) {
            return Cache::tags(Config::get('holo.allowed_setting_values_table'))->remember(
                $cacheKey,
                Config::get('holo.cache.ttl', 60),
                function () {
    ...
    public function save(array $options = [])
    {
        ...
        if (Cache::getStore() instanceof TaggableStore) {
            Cache::tags(Config::get('holo.settings_table'))->flush();
        }
        return true;
    }
